...
def extract_bits_and_combine(int_array, selected_bits):
    assert len(int_array) * len(selected_bits) <= 32
    res = 0
...
# Construct a lookup table for a given neural net distinguisher
...
if __name__ == "__main__":
    ...
    saved_path = 'lookup_table/6/21-20_{}_student_distinguisher.npy'.format(diff_pos)
    gen_lookup_table_from_net(net_path, selected_bits, saved_path)

    # 7-round distinguisher
    diff_pos = 62
    selected_bits = [12,11,4,3,1,0]
    net_path = 'saved_model/7/12-11-4-3-1-0_{}_student_distinguisher.h5'.format(diff_pos)
    saved_path = 'lookup_table/7/12-11-4-3-1-0_{}_student_distinguisher.h5.npy'.format(diff_pos)
    gen_lookup_table_from_net(net_path, selected_bits, saved_path)
