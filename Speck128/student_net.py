...
def make_resnet(num_blocks=2, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=64, ks=5, depth=5, reg_param=0.0001, final_activation='sigmoid'):
...
def extract_sensitive_bits(raw_x, bits=[11,10,9,8]):
...
def train_speck_distinguisher(num_epochs, num_rounds=5, depth=1, diff=(0x80, 0x0), version=128, bits=[11,10,9,8], folder='./'):
    # generate training data
    raw_x, raw_y = sp.make_train_data(10**7, num_rounds, diff=diff, version=version)
    extracted_x = extract_sensitive_bits(raw_x, bits=bits)

    # generate eval data
    raw_x_eval, raw_y_eval = sp.make_train_data(10**6, num_rounds, diff=diff, version=version)
    ...
    # save model
    net.save(folder + 'student_distinguisher.h5')
